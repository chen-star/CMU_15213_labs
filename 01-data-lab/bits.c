...
int allOddBits(int x) {
    /* construct 0xAAAAAAAA */
    int allOddNum = (0xAA) | (0xAA << 8) | (0xAA << 16) | (0xAA << 24);
    return !((allOddNum & x) ^ allOddNum);
}
...
int isLessOrEqual(int x, int y) {
    ...
    int result1 = signx & (!signy);

    /* if x == y */
    int result2 = !(x ^ y);

    /* if x and y have same sign */
    int result3 = !(signx ^ signy) & ((x + (~y + 1)) >> 31 & 1);
